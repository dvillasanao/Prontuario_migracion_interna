...
saveRDS(Migrantes, file = paste0(here::here(), "/Output/Municipio/06_Zonas Metropolitanas/2015/01_Migracion reciente/Matriz de migracion reciente a nivel intramunicipal.RDS"))
save(Migrantes, file = paste0(here::here(), "/Output/Municipio/06_Zonas Metropolitanas/2015/01_Migracion reciente/Matriz de migracion reciente a nivel intramunicipal.RData"))
...
saveWorkbook(wb, file = paste0(here::here(), "/Bases/Municipio/06_Zonas Metropolitanas/2015/01_Migracion reciente/Matriz de migracion reciente a nivel intramunicipal.xlsx"), overwrite = TRUE)
```
...
```{r, echo = FALSE}
load(paste0(here::here(), "/Output/Municipio/06_Zonas Metropolitanas/2015/01_Migracion reciente/Matriz de migracion reciente a nivel intramunicipal.RData"))
...
saveRDS(MR, file = paste0(here::here(), "/Output/Municipio/06_Zonas Metropolitanas/2015/01_Migracion reciente/Matrices de MR a nivel intramunicipal por ZM.RDS"))
...
saveWorkbook(wb, 
              file = paste0(here::here(), "/Bases/Municipio/06_Zonas Metropolitanas/2015/01_Migracion reciente/Matrices de MR a nivel intramunicipal por ZM.xlsx"), 
               overwrite = TRUE)
}
```
...
<div style="height:500px;overflow:auto;">
```{r, echo = FALSE}
MR <- readRDS(paste0(here::here(), "/Output/Municipio/06_Zonas Metropolitanas/2015/01_Migracion reciente/Matrices de MR a nivel intramunicipal por ZM.RDS"))
...
rownames <- Migrantes %>% 
             mutate(CVE_MUN = substr(.$CVE_MUN, 9, 16)) %>%
              left_join(., ZM_2015 %>% select(CVE_MUN, NOM_MUN), by = c("CVE_MUN")) %>%
...
circos.clear()
pdf(paste0(here::here(), "/Graficos/Municipio/06_Zonas Metropolitanas/2015/01_Migracion reciente/MR Intramunicipal x ZM.pdf"), width = 25, height = 25)
...
path = paste0(here::here(), "/Graficos/Municipio/06_Zonas Metropolitanas/2015/01_Migracion reciente/MR_Estado_alluvial_por ZM_Absolutos.pdf")
...
colnames <- row.names(Migrantes) %>% 
             as.data.frame() %>%
              rename("CVE_MUN" = ".") %>%
               right_join(., ZM_2015 %>% select(CVE_MUN, NOM_MUN), by = c("CVE_MUN")) %>%
                mutate(CVE_MUN = stringr::str_wrap(paste(.$CVE_MUN, .$NOM_MUN), 30)) %>%
                 pull(CVE_MUN)
...
circos.clear()
pdf(paste0(here::here(), "/Graficos/Municipio/06_Zonas Metropolitanas/2015/01_Migracion reciente/ChordDiagram de MR5a de ZMVM.pdf"), width = 18, height = 15)
circos.par(start.degree = 90, 
           gap.degree = 3, 
...
circos.clear()
pdf(paste0(here::here(), "/Graficos/Municipio/06_Zonas Metropolitanas/2015/01_Migracion reciente/ChordDiagram de MR5a de ZMVM_a.pdf"), width = 18, height = 15)
circos.par(start.degree = 90, 
           gap.degree = 3, 
...
circos.trackPlotRegion(track.index = 2,
                       ...
                       panel.fun = function(x, y) {
                                                 ...
                                                 # Add graduation on axis
                                                  circos.axis(h = "top",
                                                              labels = c(0, 1, 2, 3, 4, 5, 10, 20, 50, 70, 100, 500, 1000, 5000, 10000,30000,50000,100000, 200000),
                                                              major.tick.length = 0.5,
                                                              minor.ticks = 4, 
                                                              labels.cex = fontsize(12),
                                                              sector.index = sector.name,
...
path = paste0(here::here(), "/Graficos/Municipio/06_Zonas Metropolitanas/2015/01_Migracion reciente/MR_alluvial_por ZMVM.pdf")
...
################################################################################
############################## Residentes ######################################
load(file = paste0(here::here(), "/Output/Municipio/06_Zonas Metropolitanas/2015/01_Migracion reciente/Matriz de migracion reciente a nivel intramunicipal.RData"))
...
write.xlsx(tabla, file = paste0(here::here(), "/Output/Municipio/06_Zonas Metropolitanas/2015/01_Migracion reciente/Indicadores de migracion reciente x Zonas Metropolitanas.xlsx"), overwrite = TRUE)
```
