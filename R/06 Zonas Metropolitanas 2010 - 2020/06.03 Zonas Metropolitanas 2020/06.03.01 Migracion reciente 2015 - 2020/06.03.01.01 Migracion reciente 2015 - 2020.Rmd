...
cairo_pdf(paste0(here::here(), "/Graficos/Municipio/06_Zonas Metropolitanas/2020/01_Migracion reciente/ChordDiagram de MR5a desagregado por ZM (Intramunicipal).pdf"), 
          width = 15,
          height = 10, 
          family = "Montserrat Medium",
          fallback_resolution = 400,
...
cairo_pdf(paste0(here::here(), "/Graficos/Municipio/06_Zonas Metropolitanas/2020/01_Migracion reciente/Etiquetas ZM a nivel intramunicipal.pdf"), 
          width = 7, height = 8, 
          fallback_resolution = 400,
          family = "montserrat", onefile = TRUE)
for(i in 1:length(ZM_CF)){
...
cairo_pdf(paste0(here::here(), "/Graficos/Municipio/06_Zonas Metropolitanas/2020/01_Migracion reciente/ChordDiagram de MR5a desagregado por ZM (Intermunicipal).pdf"), 
          width = 15,
          height = 10, 
          family = "Montserrat Medium",
          fallback_resolution = 400,
...
cairo_pdf(paste0(here::here(), "/Graficos/Municipio/06_Zonas Metropolitanas/2020/01_Migracion reciente/Etiquetas ZM a nivel intermunicipal.pdf"), 
          width = 7, height = 9, 
          fallback_resolution = 400,
          family = "montserrat", onefile = TRUE)
for(i in 1:length(ZM_CF)){
...
## Migración metropolitana {.tabset .tabset-pills}
...
cairo_pdf(paste0(here::here(), "/Graficos/Municipio/06_Zonas Metropolitanas/2020/01_Migracion reciente/Etiquetas ZM a nivel metropolitano.pdf"), 
          width = 7, height = 8, 
          fallback_resolution = 400,
          family = "montserrat", onefile = TRUE)
for(i in 1:length(ZM_CF)){
...
################################################################################
########################### Residentes #########################################
load(file = paste0(here::here(), "/Output/Municipio/06_Zonas Metropolitanas/2020/01_Migracion reciente/Matriz de MR5a a nivel metropolitano 2020.RData"))

Residentes <- Migrantes %>%
               rownames_to_column() %>%
                gather(CVE_ZM, Value, -rowname)%>%
                 filter(rowname == CVE_ZM) %>%
                  select(-rowname) %>%
                   droplevels() %>%
                    rename("Residentes" = "Value") 
...
Inmigrantes <- Migrantes %>% 
                as.data.frame() %>%
                 tibble::rownames_to_column(var = "CVE_ZM") %>%
                  melt(., id.vars = "CVE_ZM", variable.name = "CVE_ZM_RES") %>%
                   mutate_at(vars(3), as.numeric) %>%
                    as_tibble() %>%
                     filter(CVE_ZM != CVE_ZM_RES) %>%
                      group_by(CVE_ZM) %>%
                       summarise(Inmigrantes = sum(value, na.rm = TRUE))
