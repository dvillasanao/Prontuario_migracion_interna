           ...
           # Zonas Metropolitanas por residenicia
            ...
            # Zonas Metropolitanas en el lugar de residencia hace 5 años
             left_join(., ZM_2020 %>% select(-CVE_ENT, -NOM_ENT, -NOM_MUN) %>% 
                           rename("CVE_ZM_TRABAJO15" = "CVE_ZM",
                                  "ZM_RES15" = "NOM_ZM",
                                  "MC_RES15" = "MC",
...
cairo_pdf(paste0(here::here(), "/Graficos/Municipio/06_Zonas Metropolitanas/2020/03_Movilidad laboral/ChordDiagram de MTrab desagregado por ZM.pdf"), width = 7, height = 7, 
                                         ...
                                         theme(plot.margin = margin(t = 1, r = 1.5, b = 1, l = 0, "cm"),
                                          ...
                                          scale_fill_viridis_d(option = "A") + 
                                           guides(fill = guide_legend(ncol = 1)) + 
                                            labs(fill = stringr::str_wrap(ZM_CF[x], 50),
                                             color = stringr::str_wrap(ZM_CF[x], 50))
                         leg <- get_legend(p)
                         as_ggplot(leg)
 })
path = paste0(here::here(), "/Graficos/Municipio/06_Zonas Metropolitanas/2020/03_Movilidad laboral/Etiquetas a nivel municipal.pdf")
              ...
              filter(.$rn %in% ZM | .$cn %in% ZM) %>%
               mutate(value = ifelse((.$rn != .$cn) & (.$rn %in% ZM| .$cn %in% ZM), value, 0)) %>% 
                mutate(rn = case_when(.$rn %in% ZM & .$rn %in% filtro  ~ .$rn,
                                      .$rn %in% ZM & .$rn %nin% filtro ~ str_wrap(paste(estados[as.numeric(substr(.$rn, 1, 3))], "ZMVM"), 30),
                                      .$rn %nin% ZM & substr(.$rn, 1, 3) %in% filtro_est ~ str_wrap(paste0(nom_estados[as.numeric(substr(.$rn, 1, 3))]), 30),
                                      .$rn %nin% ZM & substr(.$rn, 1, 3) %nin% filtro_est ~ "Otros estados"),
                                                 
                       cn = case_when(.$cn %in% ZM & .$cn %in% filtro  ~ .$cn,
                                      .$cn %in% ZM & .$cn %nin% filtro ~ str_wrap(paste(estados[as.numeric(substr(.$cn, 1, 3))], "ZMVM"), 30),
                                      .$cn %nin% ZM & substr(.$cn, 1, 3) %in% filtro_est ~ str_wrap(paste0(nom_estados[as.numeric(substr(.$cn, 1, 3))]), 30),
...
circos.clear()
pdf(paste0(here::here(), "/Graficos/Municipio/06_Zonas Metropolitanas/2020/03_Movilidad laboral/ChordDiagram de MTrab de ZMVM (Municipal).pdf"), width = 22, height = 20)
circos.par(start.degree = 90, 
           gap.degree = 3, 
              ...
              filter(.$rn %in% ZM | .$cn %in% ZM) %>%
               mutate(value = ifelse((.$rn != .$cn) & (.$rn %in% ZM| .$cn %in% ZM), value, 0)) %>% 
                mutate(rn = case_when(.$rn %in% ZM & .$rn %in% filtro  ~ .$rn,
                                      ...
                                      .$rn %nin% ZM & substr(.$rn, 1, 3) %in% filtro_est ~ str_wrap(paste0(nom_estados[as.numeric(substr(.$rn, 1, 3))]), 20),
                                      .$rn %nin% ZM & substr(.$rn, 1, 3) %nin% filtro_est ~ "Otros estados"),
                                                 
                       cn = case_when(.$cn %in% ZM & .$cn %in% filtro  ~ .$cn,
                                      .$cn %in% ZM & .$cn %nin% filtro ~ str_wrap(paste(estados[as.numeric(substr(.$cn, 1, 3))], "ZMVM"), 20),
                                      .$cn %nin% ZM & substr(.$cn, 1, 3) %in% filtro_est ~ str_wrap(paste0(nom_estados[as.numeric(substr(.$cn, 1, 3))]), 20),
                                      .$cn %nin% ZM & substr(.$cn, 1, 3) %nin% filtro_est ~ "Otros estados")) %>%
                 filter(value > 0)  
...
# Grupo 4
grupo4 <- tabla %>%
           filter(substr(.$rn, 1, 3) %nin% c("009", "013", "015")) %>%
            pull(rn) %>%
             unique()
...
circos.clear()
pdf(paste0(here::here(), "/Graficos/Municipio/06_Zonas Metropolitanas/2020/03_Movilidad laboral/ChordDiagram de MTrab de ZMVM_grupos (Municipal).pdf"), width = 22, height = 20)
circos.par(start.degree = 90, 
           gap.degree = 3, 
...
highlight.sector(grupo4, 
                 track.index = 1, 
                 col = groupColors[nrow(tabla1) - 10], 
...
path = paste0(here::here(), "/Graficos/Municipio/06_Zonas Metropolitanas/2020/03_Movilidad laboral/Etiquetas ZMVM a nivel municipal.pdf")
ggexport(etiquetas, width = 5, height = 5, dpi = 400, filename = path)
```
...
```{r, echo = FALSE}
load(file = paste0(here::here(), "/Output/Municipio/06_Zonas Metropolitanas/2020/03_Movilidad laboral/Indicadores de MTrab por ZM 2020 (Municipal).RData"))
tabla[1:20,] %>%  
 as.data.frame() %>%
  gt() %>% 
...
rownames <- rownames(Migrantes) %>% 
             as.data.frame() %>%
              rename("CVE_MUN" = ".") %>%
               left_join(., ZM_2020 %>% select(CVE_MUN, NOM_MUN)) %>%
                mutate(CVE_MUN = paste(.$CVE_MUN, .$NOM_MUN)) %>%
                 pull(CVE_MUN)
...
colnames <- colnames(Migrantes) %>% 
             as.data.frame() %>%
              rename("CVE_MUN" = ".") %>%
               right_join(., ZM_2020 %>% select(CVE_MUN, NOM_MUN)) %>%
                mutate(CVE_MUN = paste(.$CVE_MUN, .$NOM_MUN)) %>%
                 pull(CVE_MUN)
                                   ...
                                   tibble::rownames_to_column(var = "rn") %>% 
                                    melt(., id.vars = "rn", variable.name = "cn") %>%
                                     mutate_if(is.factor, as.character) %>%
                                      filter(.$rn %in% ZM[[x]] | .$cn %in% ZM[[x]]) %>%
                                       mutate(value = ifelse((.$rn != .$cn) & (.$rn %in% ZM[[x]] | .$cn %in% ZM[[x]]), value, 0)) %>% 
                                        mutate(rn = ifelse(.$rn %in% filtro, stringr::str_wrap(paste0(substr(as.character(.$rn), 1, 3), " Otros municipios (", estados[as.numeric(substr(as.character(.$rn), 1, 3))], ")"), 40), .$rn),
                                               cn = ifelse(.$cn %in% filtro, stringr::str_wrap(paste0(substr(as.character(.$cn), 1, 3), " Otros municipios (", estados[as.numeric(substr(as.character(.$cn), 1, 3))], ")"), 40), .$cn)) %>%
                                         filter(value > 0) %>%
                                          dcast(., rn ~ cn, value.var = "value", sum,  na.rm = TRUE) %>%
                                           column_to_rownames(., var = "rn")
})
...
tabla2 <- lapply(1:length(ZM_CF), function(x){
                                    # Paleta de colores
                                     groupColors <- setNames(viridis::viridis(length(unique(c(colnames(tabla1[[x]]), rownames(tabla1[[x]])))), option = "A", end = 0.9, begin = 0.2), 
                                                             nm = unique(c(colnames(tabla1[[x]]), rownames(tabla1[[x]]))))
                                      circos.clear()
                                      chordDiagram(x  = tabla1[[x]] %>% as.matrix(),
                                                    grid.col = groupColors) %>%
...
```{r, eval = FALSE, results='hide'}
pdf(paste0(here::here(), "/Graficos/Municipio/06_Zonas Metropolitanas/2020/03_Movilidad laboral/ChordDiagram de MTrab desagregado por ZM  (Intramunicipal).pdf"), width = 25, height = 22)
for(i in 1:length(ZM_CF)){
circos.clear()  
groupColors <- setNames(viridis::viridis(length(unique(c(colnames(tabla1[[i]]), rownames(tabla1[[i]])))), option = "A", end = 0.9, begin = 0.2), 
                        nm = unique(c(colnames(tabla1[[i]]), rownames(tabla1[[i]]))))
...
# Paleta de colores
groupColors <- viridis::viridis(max(c(ncol(tabla1[[i]]), nrow(tabla1[[i]]))), option = "A", end = 0.9, begin = 0.2)
...
path = paste0(here::here(), "/Graficos/Municipio/06_Zonas Metropolitanas/2020/03_Movilidad laboral/Etiquetas ZM a nivel intramunicipal.pdf")
ggexport(etiquetas, width = 5, height = 5, dpi = 400, filename = path)
```
...
colnames <- row.names(Migrantes) %>% 
             as.data.frame() %>%
              rename("CVE_MUN" = ".") %>%
               right_join(., ZM_2020 %>% select(CVE_MUN, NOM_MUN), by = c("CVE_MUN")) %>%
                mutate(CVE_MUN = stringr::str_wrap(paste(.$CVE_MUN, .$NOM_MUN), 30)) %>%
                 pull(CVE_MUN)
           ...
           as.data.frame() %>%
            tibble::rownames_to_column(var = "rn") %>% 
             melt(., id.vars = "rn", variable.name = "cn") %>%
              mutate_if(is.factor, as.character) %>%
               mutate(value = ifelse((.$rn != .$cn) & (.$rn %in% ZM | .$cn %in% ZM), value, 0)) %>% 
                mutate(rn = ifelse(.$rn %in% filtro, stringr::str_wrap(paste0(substr(as.character(.$rn), 1, 3), " Otros municipios (", estados[as.numeric(substr(as.character(.$rn), 1, 3))], ")"), 30), .$rn),
                       cn = ifelse(.$cn %in% filtro, stringr::str_wrap(paste0(substr(as.character(.$cn), 1, 3), " Otros municipios (", estados[as.numeric(substr(as.character(.$cn), 1, 3))], ")"), 30), .$cn)) %>%
                 filter(value > 0) %>%
                  dcast(., rn ~ cn, value.var = "value", sum,  na.rm = TRUE) %>%
                   column_to_rownames(., var = "rn")
```
...
circos.clear()
pdf(paste0(here::here(), "/Graficos/Municipio/06_Zonas Metropolitanas/2020/03_Movilidad laboral/ChordDiagram de MTrab de ZMVM (Intramunicipal).pdf"), width = 22, height = 20)
circos.par(start.degree = 90, 
           gap.degree = 3, 
           track.margin = c(-0.1, 0.1), 
           points.overflow.warning = FALSE)
...
circos.clear()
pdf(paste0(here::here(), "/Graficos/Municipio/06_Zonas Metropolitanas/2020/03_Movilidad laboral/ChordDiagram de MTrab de ZMVM_grupos (Intramunicipal).pdf"), width = 22, height = 20)
circos.par(start.degree = 90, 
           gap.degree = 3, 
           track.margin = c(-0.2, 0.2), 
           points.overflow.warning = FALSE)
...
path = paste0(here::here(), "/Graficos/Municipio/06_Zonas Metropolitanas/2020/03_Movilidad laboral/Etiquetas ZMVM a nivel intramunicipal.pdf")
ggexport(etiquetas, width = 5, height = 5, dpi = 400, filename = path)
```
...
```{r, echo = FALSE}
load(file = paste0(here::here(), "/Output/Municipio/06_Zonas Metropolitanas/2020/03_Movilidad laboral/Indicadores de MTrab por ZM 2020 (Intramunicipal).RData"))
tabla[1:20,] %>%  
 as.data.frame() %>%
  gt() %>% 
   tab_header(title = "Indicadores de  movilidad laboral",  
                                       ...
                                       tibble::rownames_to_column(var = "rn") %>% 
                                        melt(., id.vars = "rn", variable.name = "cn") %>%
                                         mutate_if(is.factor, as.character) %>%
                                          filter(.$rn %in% ZM[[x]] | .$cn %in% ZM[[x]]) %>%
                                           mutate(value = ifelse((.$rn != .$cn) & (.$rn %in% ZM[[x]] | .$cn %in% ZM[[x]]), value, 0)) %>% 
                                            mutate(rn = case_when(.$rn %in% ZM[[x]] & .$rn %in% filtro  ~ .$rn,
                                                                  .$rn %in% ZM[[x]] & .$rn %nin% filtro ~ str_wrap(paste(estados[as.numeric(substr(.$rn, 1, 3))], "ZMVM"), 50),
                                                                  .$rn %nin% ZM[[x]] & substr(.$rn, 1, 3) %in% filtro_est ~ str_wrap(paste0(nom_estados[as.numeric(substr(.$rn, 1, 3))]), 50),
                                                                  .$rn %nin% ZM[[x]] & substr(.$rn, 1, 3) %nin% filtro_est ~ "Otros estados"),
                                                 
                                                   cn = case_when(.$cn %in% ZM[[x]] & .$cn %in% filtro  ~ .$cn,
                                                                  .$cn %in% ZM[[x]] & .$cn %nin% filtro ~ str_wrap(paste(estados[as.numeric(substr(.$cn, 1, 3))], "ZMVM"), 50),
                                                                  .$cn %nin% ZM[[x]] & substr(.$cn, 1, 3) %in% filtro_est ~ str_wrap(paste0(nom_estados[as.numeric(substr(.$cn, 1, 3))]), 50),
                                                                  .$cn %nin% ZM[[x]] & substr(.$cn, 1, 3) %nin% filtro_est ~ "Otros estados")) %>%
                                            filter(value > 0) %>%
                                             dcast(., rn ~ cn, value.var = "value", sum,  na.rm = TRUE) %>%
                                              column_to_rownames(., var = "rn") 
...
tabla2 <- lapply(1:length(ZM), function(x){
                       # Paleta de colores
...
```{r, eval = FALSE, results='hide'}
pdf(paste0(here::here(), "/Graficos/Municipio/06_Zonas Metropolitanas/2020/03_Movilidad laboral/ChordDiagram de MTrab desagregado por ZM  (Intermunicipal).pdf"), width = 22, height = 20)
for(i in 1:length(ZM_CF)){
...
path = paste0(here::here(), "/Graficos/Municipio/06_Zonas Metropolitanas/2020/03_Movilidad laboral/Etiquetas ZM a nivel intermunicipal.pdf")
ggexport(etiquetas, width = 5, height = 5, dpi = 400, filename = path)
```
               ...
               filter(.$rn %in% ZM | .$cn %in% ZM) %>% 
                mutate(value = ifelse((.$rn != .$cn) & (.$rn %in% ZM | .$cn %in% ZM), value, 0)) %>% 
                 mutate(rn = case_when(.$rn %in% ZM & .$rn %in% filtro  ~ .$rn,
                                       ...
                                       .$rn %nin% ZM & .$rn %nin% filtro & substr(.$rn, 1, 3) %in% filtro_est ~ str_wrap(paste0(nom_estados[as.numeric(substr(.$rn, 1, 3))]), 50),
                        ...
                        cn = case_when(.$cn %in% ZM & .$cn %in% filtro  ~ .$cn,
                                       ...
                                       .$cn %nin% ZM & .$cn %nin% filtro & substr(.$cn, 1, 3) %in% filtro_est ~ str_wrap(paste0(nom_estados[as.numeric(substr(.$cn, 1, 3))]), 50),
...
```{r}
pdf(paste0(here::here(), "/Graficos/Municipio/06_Zonas Metropolitanas/2020/03_Movilidad laboral/ChordDiagram de MTrab de ZMVM (Intermunicipal).pdf"), width = 20, height = 22)
circos.clear()
circos.par(start.degree = 90, 
           gap.degree = 3, 
           track.margin = c(-0.1, 0.1), 
...
# Paleta de colores
groupColors <- setNames(viridis::viridis(length(unique(c(colnames(tabla1), rownames(tabla1)))), option = "A"), 
...
# Grupo 4
grupo4 <- tabla %>%
           filter(substr(.$rn, 1, 3) %nin% c("009", "015", "013")) %>%
            pull(rn) %>%
             unique()
...
pdf(paste0(here::here(), "/Graficos/Municipio/06_Zonas Metropolitanas/2020/03_Movilidad laboral/ChordDiagram de MTrab de ZMVM_grupos (Intermunicipal).pdf"), width = 22, height = 20)
circos.clear()
circos.par(start.degree = 90, 
           gap.degree = 3, 
           track.margin = c(-0.2, 0.2), 
...
path = paste0(here::here(), "/Graficos/Municipio/06_Zonas Metropolitanas/2020/03_Movilidad laboral/Etiquetas ZMVM a nivel intermunicipal.pdf")
...
```{r, echo = FALSE}
load(file = paste0(here::here(), "/Output/Municipio/06_Zonas Metropolitanas/2020/03_Movilidad laboral/Indicadores de MTrab por ZM 2020 (Intermunicipal).RData"))
tabla[1:20,] %>%  
 as.data.frame() %>%
  gt() %>% 
   tab_header(title = "Indicadores de  movilidad laboral",  
...
```{r, eval = FALSE, results='hide'}
pdf(paste0(here::here(), "/Graficos/Municipio/06_Zonas Metropolitanas/2020/03_Movilidad laboral/ChordDiagram de MTrab desagregado por ZM (metropolitano).pdf"), width = 25, height = 27)
for(i in 1){
circos.clear()
circos.par(start.degree = 90, 
           gap.degree = 3, 
...
```{r, eval = FALSE, results='hide'}
pdf(paste0(here::here(), "/Graficos/Municipio/06_Zonas Metropolitanas/2020/03_Movilidad laboral/ChordDiagram de MEst desagregado por ZM (metropolitano)_individual.pdf"), width = 27, height = 25)
for(i in 1:length(ZM)){
circos.clear()
circos.par(start.degree = 90, 
           gap.degree = 3, 
...
```{r}
 etiquetas  <- lapply(1:length(ZM), function(x){
                        p <-  tabla1[[x]] %>% 
                                  as.data.frame() %>%
                                   tibble::rownames_to_column(var = "rn") %>%
                                    melt(., id.vars = "rn", variable.name = "cn") %>%
                                       ...
                                       geom_bar(aes(x = value, fill = rn))  + 
                                        geom_bar(aes(x = value, fill = cn))  + 
                                         theme(plot.margin = margin(t = 1, r = 1.5, b = 1, l = 0, "cm"),
                                          ...
                                          scale_fill_viridis_d(option = "A") + 
                                           guides(fill = guide_legend(ncol = 1)) + 
                                            labs(fill = stringr::str_wrap(ZM[x], 100),
                                             color = stringr::str_wrap(ZM[x], 100))
                         leg <- get_legend(p)
                         as_ggplot(leg)
 })
path = paste0(here::here(), "/Graficos/Municipio/06_Zonas Metropolitanas/2020/03_Movilidad laboral/Etiquetas ZM a nivel metropolitano.pdf")
ggexport(etiquetas, width = 5, height = 5, dpi = 400, filename = path)
```
