...
x <- 2
union(rownames(tabla1[[x]]), colnames(tabla1[[x]]))
...
tabla2 <- lapply(1:length(ZM_CF), function(x){
                    # Paleta de colores
                       groupColors <- setNames(colorRampPalette(paleta)(length(unique(c(colnames(tabla1[[x]]), rownames(tabla1[[x]]))))),
                        ...
                        circos.clear()
                         chordDiagram(x  = tabla1[[x]] %>% as.matrix(), 
                                      order = union(rownames(tabla1[[x]]), colnames(tabla1[[x]])),
                                       transparency = 0.4,
                                        grid.col = groupColors)  %>%
                          pull(col)
  }
...
```{r, eval = FALSE, results='hide'}
cairo_pdf(paste0(here::here(), "/Graficos/Municipio/06_Zonas Metropolitanas/2020/03_Movilidad laboral/ChordDiagram de MTrab desagregado por ZM  (Intermunicipal).pdf"), width = 7, height = 7, 
          fallback_resolution = 400,
          family = "montserrat", onefile = TRUE)
for(i in 1:length(ZM_CF)){
# Paleta de colores
...
```{r, eval = FALSE, results='hide'}
cairo_pdf(paste0(here::here(), "/Graficos/Municipio/06_Zonas Metropolitanas/2020/03_Movilidad laboral/ChordDiagram de MTrab desagregado por ZM (metropolitano).pdf"), width = 9, height = 9, 
          fallback_resolution = 400,
          family = "montserrat", onefile = TRUE)
for(i in 1){
circos.clear()
...
```{r, eval = FALSE, results='hide'}
cairo_pdf(paste0(here::here(), "/Graficos/Municipio/06_Zonas Metropolitanas/2020/03_Movilidad laboral/ChordDiagram de MEst desagregado por ZM (metropolitano)_individual.pdf"), width = 9, height = 9, 
          fallback_resolution = 400,
          family = "montserrat", onefile = TRUE)
for(i in 1:length(ZM)){
circos.clear()
