---
title: "Migración de retorno 2020"
author: "CONAPO"
output:
   html_document:
      highlight: tango
...
# CVE_ENT, ENT, MUN, CVE_MUN, ENT_PAIS_NAC, ENT_PAIS_RES_5A, MUN_RES_5A, CVE_MUN_RES15
mydata <- data %>%
           select(CVE_ENT, ENT, MUN, CVE_MUN, ENT_PAIS_NAC, ENT_PAIS_RES_5A, MUN_RES_5A, CVE_MUN_RES15, EDAD, 
                  SEXO, AFRODES, HLENGUA, QDIALECT_INALI, ELENGUA, PERTE_INDIGENA, ALFABET, ASISTEN, NIVACAD, ALFABET, 
                  CAUSA_MIG, SITUA_CONYUGAL, HIJOS_NAC_VIVOS, CONACT, OCUPACION_C, FACTOR, ESTRATO, UPM) %>%
            mutate(ENT_PAIS_NAC = case_when(.$ENT_PAIS_NAC %in% tablas ~.$ENT_PAIS_NAC,
                    ...
                    ENT_PAIS_RES_5A = case_when(.$ENT_PAIS_RES_5A %in% tablas ~.$ENT_PAIS_RES_5A,
                                                .$ENT_PAIS_RES_5A %in% "" ~ "0",
                                                .$ENT_PAIS_RES_5A %in% "997" ~ "997",
                                                .$ENT_PAIS_RES_5A %in% "998" ~ "998",
                                                .$ENT_PAIS_RES_5A %in% "999" ~ "999",
                                                .$ENT_PAIS_RES_5A %nin% c(tablas, "997", "998", "999") ~ "888" #Residencia en otro país
                                                )) %>% 
              mutate(I_Condicion_Nacimiento = case_when(.$ENT_PAIS_NAC  == "888" ~ "888",
                     ...
                     I_Condicion_Residencia2015 = case_when(.$ENT_PAIS_RES_5A == "0" ~ "0", # Población que no entra en esta categoría
                                                            .$ENT_PAIS_RES_5A == "888" ~ "888",
                                                            .$ENT_PAIS_RES_5A == "997" ~ "997",
                                                            .$ENT_PAIS_RES_5A == "998" ~ "998",
                                                            .$ENT_PAIS_RES_5A == "999" ~ "999",
                                                            .$CVE_ENT == .$ENT_PAIS_RES_5A ~ "1",
                                                            .$CVE_ENT != .$ENT_PAIS_RES_5A ~ "2",
                                                             TRUE ~ "0")) %>%
################################################# Indicador de migración de retorno #######################################################
               mutate(I_Migrante_Retorno = case_when(.$I_Condicion_Residencia2015 == "2" & .$I_Condicion_Nacimiento == "1" ~ "Migrante de retorno",
                                                     .$I_Condicion_Residencia2015 == "2" & .$I_Condicion_Nacimiento == "2" ~ "Inmigrante",
                                                     .$I_Condicion_Residencia2015 == "2" & .$I_Condicion_Nacimiento == "888" ~ "De otro país",   
                                                     .$I_Condicion_Residencia2015 == "2" & .$I_Condicion_Nacimiento == "997" ~ "No especificó de entidad federativa",                                                        .$I_Condicion_Residencia2015 == "2" & .$I_Condicion_Nacimiento == "998" ~ "No especificó el país",
                                                     .$I_Condicion_Residencia2015 == "2" & .$I_Condicion_Nacimiento == "999" ~ "No especificado", 
                                                      TRUE ~ "0")) 

save(mydata, file = paste0(here::here(),"/Bases/05_Migracion de retorno_2020.RData"))
```
...
mydata <- mydata %>%
           select(CVE_ENT, ENT, MUN, CVE_MUN, ENT_PAIS_NAC, ENT_PAIS_RES_5A, MUN_RES_5A, CVE_MUN_RES15, EDAD, 
                   I_Condicion_Nacimiento, I_Condicion_Residencia2015, I_Migrante_Retorno, FACTOR, ESTRATO, UPM) %>%
           mutate(M = 1)  %>%
...
saveRDS(MC, file = paste0(here::here(), "/Output/Estado/05_Migracion de retorno/2020/MC_estado.RDS"))
```
...
```{r}
MC <- readRDS(file = paste0(here::here(), "/Output/Estado/05_Migracion de retorno/2020/MC_estado.RDS"))
Migrantes <- svytable(~ENT_PAIS_RES_5A + CVE_ENT, design = MC) 
```
...
Migrantes <- Migrantes %>%
              as.data.frame() %>%
               expss::cross_cases(CVE_ENT, ENT_PAIS_RES_5A, weight = Freq) %>%
                as.data.frame() %>%
                 slice(-33) %>% 
                  select(-row_labels) #%>%
...
wb <- createWorkbook()
addWorksheet(wb, "MRt. 2020")
writeData(wb, 1, Migrantes, colNames = TRUE, rowNames = TRUE)
saveWorkbook(wb, file = paste0(here::here(), "/Bases/Estado/05_Migracion de retorno/2020/Matriz de MRt a nivel estatal 2020.xlsx"), overwrite = TRUE)

save(Migrantes, file = paste0(here::here(), "/Bases/Estado/05_Migracion de retorno/2020/Matriz de MRt a nivel estatal 2020.RData"))
```
...
```{r, echo = FALSE}
load(file = paste0(here::here(), "/Bases/Estado/05_Migracion de retorno/2020/Matriz de MRt a nivel estatal 2020.RData"))
...
tabla %>%  
 gt() %>% 
  tab_header(title = "Matriz de migración de retorno, 2020", 
...
p
require(htmlwidgets)
htmlwidgets::saveWidget(p, paste0(here::here(),"/Graficos/Estado/05_Migracion de retorno/2020/MRt a nivel estatal 2020.html"), selfcontained = TRUE)
#require(webshot)
#webshot(url = paste0(here::here(),"/Graficos/Estado/05_Migracion de retorno/2020/MRt a nivel estatal 2020.html"),
 #         file = paste0(here::here(),"/Graficos/Estado/05_Migracion de retorno/2020/MRt a nivel estatal 2020.png"),
  #                cliprect = "viewport")
```
...
circos.clear()
cairo_pdf(paste0(here::here(),"/Graficos/Estado/05_Migracion de retorno/2020/ChordDiagram de MRt a nivel estatal.pdf"), 
          width = 7, height = 7, 
...
#p <- data.frame(estados = est,
 #               filtro_estados = filtro_mig,
                 #NUM = 1:32)
#write.xlsx(p, file = paste0(here::here(), "/Bases/Estado/05_Migracion de retorno/2020/Filtro a nivel estatal.xlsx"))

filtro_mig <- read.xlsx(paste0(here::here(), "/Bases/Estado/05_Migracion de retorno/2020/Filtro a nivel estatal.xlsx"), colNames = TRUE) %>% 
               pull(filtro_estados)
...
for(i in 1:32){
     ...
     saveWorkbook(wb, 
                  file = paste0(here::here(), "/Output/Estado/05_Migracion de retorno/2020/Matriz MRt por estados_2020_Reduccion.xlsx"), 
               overwrite = TRUE)
}

saveRDS(tabla1, file = paste0(here::here(), "/Output/Estado/05_Migracion de retorno/2020/Tabla MRt a nivel estatal.RDS"))
```

```{r tablas de matrices}
tabla1 <- readRDS(file = paste0(here::here(), "/Output/Estado/05_Migracion de retorno/2020/Tabla MRt a nivel estatal.RDS"))
...
for(i in 1:32){
     ...
     saveWorkbook(wb, 
                  file = paste0(here::here(), "/Output/Estado/05_Migracion de retorno/2020/Matriz MRt por estados_2020_Reduccion_Totales.xlsx"), 
               overwrite = TRUE)
}
```
...
```{r, eval = FALSE}
cairo_pdf(paste0(here::here(),"/Graficos/Estado/05_Migracion de retorno/2020/ChordDiagram de MRt para cada estado.pdf"), 
          width = 8,
          height = 8, 
          family = "Montserrat Medium",
          fallback_resolution = 400,
...
cairo_pdf(paste0(here::here(), "/Graficos/Estado/05_Migracion de retorno/2020/Etiquetas a nivel estatal.pdf"),
          width = 7, height = 8, 
          fallback_resolution = 400,
          family = "Montserrat Medium", onefile = TRUE)
for(i in 1:length(estados)){
  print(etiquetas[i])
}
dev.off()
...
etiquetas  <- lapply(1, function(x){
                ...
                #Se agrupan los totales
                 full_join(
                   ...
                   rename("CVE_ENT" = "name",
                          "Entidad de residencia 5a" = "Total_Filas",
                          "Migrantes de retorno" = "Total") %>%
                  select(`Entidad de residencia 5a`, `CVE_ENT`, `Migrantes de retorno`)
                 
                 
 })
x <- colorRampPalette(paleta)(length(etiquetas[[1]]))
p <- etiquetas[[1]] %>%
 mutate(n = 1:nrow(.)) %>%
   relocate("n", .before = CVE_ENT) %>%
    gt::gt() %>% 
     data_color(columns = "n", colors = x) %>%
      fmt_integer(columns = c(1, 4), sep_mark = " ") %>%
       tab_options(data_row.padding = px(1)) %>% 
        opt_table_font( font = "Montserrat Medium", 
                        weight = 5) %>%
         cols_align(align = c("center"),
                    columns = everything()) %>%
         cols_label(
                    `Entidad de residencia 5a` = "Personas emigrantes",
                    `CVE_ENT` = "Entidad",
                    `n` = "",
                    `Migrantes de retorno` = "Migrantes de retorno"
                    )
gtsave(p, paste0(here::here(), "/Graficos/Estado/05_Migracion de retorno/2020/p.pdf"))
```
...
filtro_mig <- read.xlsx(paste0(here::here(), "/Bases/Estado/05_Migracion de retorno/2020/Filtro a nivel estatal_Retorno.xlsx"), colNames = TRUE) %>% 
               pull(filtro_estados)
...
tabla1 <- lapply(1:32, function(x){
                         ...
                         filtro <- filtro_rn %>%
                                    full_join(., filtro_cn, by = c("rn" = "cn")) %>%
                                     mutate(value = sum_row(.$Emg, na.rm = TRUE)) %>%
                                     #mutate(value = sum_row(.$Inm, .$Emg, na.rm = TRUE)) %>%
                                      filter(value > filtro_mig[x] & rn != estado[x]) %>%
                                       pull(rn)
...
for(i in 1:32){
     ...
     writeData(wb, i, tabla, colNames = TRUE, rowNames = TRUE)
     saveWorkbook(wb, 
                  file = paste0(here::here(), "/Output/Estado/05_Migracion de retorno/2020/Matriz MRt por estados_2020_GGSankey_Reduccion.xlsx"), 
               overwrite = TRUE)
}

saveRDS(tabla1, file = paste0(here::here(), "/Output/Estado/05_Migracion de retorno/2020/Tabla MRt a nivel estatal_GGSankey.RDS"))
```


```{r tablas de matrices}
tabla1 <- readRDS(file = paste0(here::here(), "/Output/Estado/05_Migracion de retorno/2020/Tabla MRt a nivel estatal_GGSankey.RDS"))
...
totales <- lapply(1:length(estados), function(x){
                             ...
                             tabla %>%
                              as.data.frame() %>%
                               adorn_totals(c("row", "col"), 
                                            fill = "-", 
                                            na.rm = TRUE, 
                                            ,,,,contains(str_sort(unique(c(colnames(tabla), rownames(tabla))), numeric = TRUE))) %>%
                                slice(nrow(.)) %>% 
                                 t() %>%
                                  as.data.frame()%>%
                                   rownames_to_column(var = "name")  %>% 
  ...
  porcentajes <- lapply(1:length(estados), function(x){
                             tabla <- tabla1[[x]] %>%
                              dcast(., rn ~ cn, value.var = "value", sum,  na.rm = TRUE) %>%                                                                           column_to_rownames(., var = "rn") 
...
for(i in 1:32){
     ...
     writeData(wb, i, porcentajes[[i]], colNames = TRUE, startCol = 5)
     saveWorkbook(wb, 
                  file = paste0(here::here(), "/Output/Estado/05_Migracion de retorno/2020/Matriz MRt por estados_2020_Reduccion_GGSankey_Totales.xlsx"), 
...
p <- lapply(1:32, function(x){
                     tabla <- tabla1[[x]] %>% 
                               filter(value > 0) %>%
                                mutate(value = ifelse(.$rn %in% estado[x] | .$cn %in% estado[x], value, 0)) %>%
                                 group_by(rn) %>%
                                  mutate(freq = value/sum(value, na.rm = TRUE))
                     tabla %>%
                     ggplot(aes(axis1 = cn, 
                                 axis2 = rn, 
                            ...
                            theme_void() + 
                             theme(plot.margin = margin(t = 1, r = 1.5, b = 1, l = 0, "cm"),
                                    text = element_text(family = "Montserrat Medium"),
                                     axis.text = element_blank(),
                                      axis.title = element_blank(),
                                       strip.text = element_text(size = 10, face = "bold", family = "Montserrat Medium"),
                                        legend.position = "none") + 
                              scale_x_discrete(expand = c(-0.1, 0.35)) +
                               scale_fill_manual(values = colorRampPalette(paleta)(length(unique(c(tabla$rn, tabla$cn))))) + 
                                guides(fill = guide_legend(ncol = 1, na.translate = F)) + 
                                 labs(fill = "", 
                                      color = "")
              }
)
path = paste0(here::here(),"/Graficos/Estado/05_Migracion de retorno/2020/GSankey de MRt a nivel estatal.pdf")
ggexport(p, width = 14, height = 10, dpi = 400, filename = path, device = "cairo_pdf")
```
...
MC <- mydata  %>%
       select(FACTOR, ESTRATO, UPM, CVE_ENT, CVE_ENT, ENT_PAIS_RES_5A, EDAD, I_Migrante_Retorno) %>%
        as.data.frame() %>%
         mutate(EDAD = as.numeric(.$EDAD)) %>%
        ...
        # Se reclasifica la variable de residencia hace 5 años
           mutate(ENT_PAIS_RES_5A = case_when(.$ENT_PAIS_RES_5A %in% tablas ~.$ENT_PAIS_RES_5A)) %>%
            mutate(M = 1) %>% # Residentes 
             mutate(Migrantes.Retorno = case_when(.$CVE_ENT !=.$ENT_PAIS_RES_5A  & (EDAD >= 5 & EDAD <= 130) & I_Migrante_Retorno %in% "Migrante de retorno" ~ .$ENT_PAIS_RES_5A),
                    Inmigrantes = case_when(.$CVE_ENT !=.$ENT_PAIS_RES_5A  & (EDAD >= 5 & EDAD <= 130) & I_Migrante_Retorno %in% "Inmigrante" ~ .$ENT_PAIS_RES_5A),
                    Residentes = case_when(.$CVE_ENT ==.$ENT_PAIS_RES_5A & (EDAD >= 5 & EDAD <= 130) ~ .$ENT_PAIS_RES_5A)) %>% 
              select(FACTOR, ESTRATO, UPM, CVE_ENT, ENT_PAIS_RES_5A, M, Migrantes.Retorno, Inmigrantes, Residentes) %>%
               svydesign(data = ., id = ~ UPM, strata = ~ESTRATO, weight = ~FACTOR, nest = T)
...
Migrantes <- svytable(~CVE_ENT + ENT_PAIS_RES_5A , design = MC) 
...
write.xlsx(tabla, file = paste0(here::here(),"/Output/Estado/05_Migracion de retorno/2020/Indicadores de migracion de retorno a nivel estatal 2020.xlsx"), overwrite = TRUE)

save(tabla, file = paste0(here::here(),"/Output/Estado/05_Migracion de retorno/2020/Indicadores de migracion de retorno a nivel estatal 2020.RData"))
```

```{r, echo = FALSE}
load(file = paste0(here::here(),"/Output/Estado/05_Migracion de retorno/2020/Indicadores de migracion de retorno a nivel estatal 2020.RData"))
...
kable(dplyr::select(tibble::as_tibble(sesion_info$packages %>% dplyr::filter(attached == TRUE)),
 ...
 kable_styling(font_size = 10, 
  ...
  kable_classic(full_width = TRUE, html_font = "Montserrat Medium") %>% 
