...
rownames(tabla) <- stringr::str_wrap(nom_estados, 50)data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAABIAAAASCAYAAABWzo5XAAAAWElEQVR42mNgGPTAxsZmJsVqQApgmGw1yApwKcQiT7phRBuCzzCSDSHGMKINIeDNmWQlA2IigKJwIssQkHdINgxfmBBtGDEBS3KCxBc7pMQgMYE5c/AXPwAwSX4lV3pTWwAAAABJRU5ErkJggg==
...
circos.clear()
cairo_pdf(paste0(here::here(),"/Graficos/Estado/05_Migracion de retorno/2015/ChordDiagram de MRt a nivel estatal.pdf"), 
          width = 7, height = 7, 
...
```{r, eval = FALSE}
cairo_pdf(paste0(here::here(),"/Graficos/Estado/05_Migracion de retorno/2015/ChordDiagram de MRt para cada estado.pdf"), 
          width = 8,
          height = 8, 
          family = "Montserrat Medium",
          fallback_resolution = 400,
...
cairo_pdf(paste0(here::here(), "/Graficos/Estado/05_Migracion de retorno/2015/Etiquetas a nivel estatal.pdf"),
          width = 7, height = 8, 
          fallback_resolution = 400,
...
Migrantes <- svytable(~ENT + ENT_PAIS_RES10, design = MC)  
...
Migrantes <- Migrantes %>%
              as.data.frame() %>%
               expss::cross_cases(ENT, ENT_PAIS_RES10, weight = Freq) %>%
                as.data.frame() %>%
                 slice(-33) %>% 
                  select(-row_labels) %>% 
                   sna::diag.remove(remove.val = 0)
