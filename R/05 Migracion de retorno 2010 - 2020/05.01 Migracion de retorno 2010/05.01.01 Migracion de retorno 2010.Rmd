            ...
            rename("CVE_ENT" = "ENT",
             ...
             mutate(ENT_PAIS_NAC = case_when(.$ENT_PAIS_NAC %in% tablas ~.$ENT_PAIS_NAC,
                    ...
                    RES05EDO_C = case_when(.$RES05EDO_C %in% tablas ~.$RES05EDO_C,
                                           ...
                                           .$RES05EDO_C %nin% c(tablas, "900", "999") ~ "888" #Residencia en otro país
                                           )) %>% 
               mutate(I_Condicion_Nacimiento = case_when(.$ENT_PAIS_NAC  == "888" ~ "888",
                      ...
                      I_Condicion_Residencia2005 = case_when(.$RES05EDO_C == "0" ~ "0", # Población que no entra en esta categoría 
                                                             .$RES05EDO_C == "888" ~ "888",
                                                             .$RES05EDO_C == "900" ~ "900",
                                                             .$RES05EDO_C == "999" ~ "999",
...
################################################# Indicador de migración de retorno #######################################################
                mutate(I_Migrante_Retorno = case_when(.$I_Condicion_Residencia2005 == "2" & .$I_Condicion_Nacimiento == "1" ~ "Migrante de retorno",
                                                      ...
                                                      .$I_Condicion_Residencia2005 == "2" & .$I_Condicion_Nacimiento == "888" ~ "De otro país",    
                                                      .$I_Condicion_Residencia2005 == "2" & .$I_Condicion_Nacimiento == "900" ~ "No especificó de entidad federativa",                                                        
...
mydata <- mydata %>%
           select(CVE_ENT, NOM_ENT, MUN, NOM_MUN, CVE_MUN, ENT_PAIS_NAC, CVE_MUN_RES, RES05EDO_C, MUN05OTR_C, EDAD, 
...
tabla %>%  
 gt() %>% 
  tab_header(title = "Matriz de migración de retorno, 2010", 
...
circos.clear()
#svglite::svglite(paste0(here::here(),"Graficos/Estado/05_Migracion de retorno/2010/ChordDiagram de MRt a nivel estatal.svg"), width = 20, height = 20)
pdf(paste0(here::here(),"/Graficos/Estado/05_Migracion de retorno/2010/ChordDiagram de MRt a nivel estatal.pdf"), width = 20, height = 20)
...
circos.clear()
pdf(paste0(here::here(),"/Graficos/Estado/05_Migracion de retorno/2010/ChordDiagram de MRt para cada estado.pdf"), width = 20, height = 20)
for(i in 1:32){
circos.par(start.degree = 90, 
           gap.degree = 3, 
           clock.wise = FALSE,
