...
circos.clear()
pdf(paste0(here::here(), "/Graficos/Estado/03_Movilidad laboral/2015/ChordDiagram de MTrab a nivel estatal.pdf"), width = 20, height = 20)
...
pdf(paste0(here::here(),"/Graficos/Estado/03_Movilidad laboral/2015/ChordDiagram de MTrab para cada estado.pdf"), width = 20, height = 20)
for(i in 1:32){
# Paleta de colores 
                ...
                mutate(CVE_MUN_TRABAJO = paste0(ENT_PAIS_TRAB, MUN_TRAB)) %>%
                 mutate(ENT_PAIS_TRAB = case_when(.$ENT_PAIS_TRAB %in% estados ~.$ENT_PAIS_TRAB,
                        ...
                        CVE_MUN_TRABAJO = case_when(.$CVE_MUN_TRABAJO %in% municipios ~.$CVE_MUN_TRABAJO,
                                                    ...
                                                    .$CVE_MUN_TRABAJO %in% "999999" ~ '999999',
                                                    .$ENT_PAIS_TRAB %in% "999" ~ "999",
                                                    .$MUN_TRAB %in% "999" ~ "999")) %>%
                  mutate(EDAD = as.numeric(.$EDAD)) %>%
                   subset((EDAD >= 15 & EDAD <=130) & (CONACT >= 10 & CONACT <= 20)) %>%
...
saveRDS(tabla2, file = paste0(here::here(), "/Output/Municipio/03_Movilidad laboral/2015/Paleta de colores MTrab a nivel municipal.RDS"))
```


```{r, eval = FALSE}
pdf(paste0(here::here(), "/Graficos/Municipio/03_Movilidad laboral/2015/ChordDiagram de MTrab desagregado por estado.pdf"), width = 20, height = 20)
for(i in 1){
...
```{r, eval = FALSE}
pdf(paste0(here::here(), "/Graficos/Municipio/03_Movilidad laboral/2015/ChordDiagram de Mtrab desagregado a nivel intramunicipal.pdf"), width = 20, height = 20)
for(i in 1:32){
# Paleta de colores
groupColors <- setNames(viridis::viridis(length(unique(c(colnames(tabla1[[i]]), rownames(tabla1[[i]])))), option = "A"), 
                        nm = unique(c(colnames(tabla1[[i]]), rownames(tabla1[[i]]))))
...
```{r, eval = FALSE}
pdf(paste0(here::here(), "/Graficos/Municipio/03_Movilidad laboral/2015/ChordDiagram de Mtrab desagregado a nivel intermunicipal.pdf"), width = 20, height = 20)
for(i in 1){
# Paleta de colores
groupColors <- setNames(viridis::viridis(length(unique(c(colnames(tabla1[[i]]), rownames(tabla1[[i]])))), option = "A"), 
                        nm = unique(c(colnames(tabla1[[i]]), rownames(tabla1[[i]]))))
