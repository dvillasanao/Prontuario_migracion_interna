...
as.integer(tabla) %>%  
 as.data.frame() %>%
  mutate(across(where(is.numeric), ~ prettyNum(., big.mark = " ", preserve.width = "none"))) %>%
   gt() %>% 
    tab_header(title = "Población ocupada de 12 años y más") %>%
...
as.integer(tabla) %>%  
 as.data.frame() %>%
  mutate(across(where(is.numeric), ~ prettyNum(., big.mark = " ", preserve.width = "none"))) %>%
   gt() %>% 
    tab_header(title = "Población economicámente activa de 12 años y más") %>%
     tab_options(heading.title.font.size = 12, 
                 heading.align = "center",
                 heading.subtitle.font.size = 10,
                 table.align = "center",
...
paleta <- rev(c("#230A3A", "#250A3A", "#2E1047", "#2A004D", "#3B004D", "#4D004B", "#501551", "#550252", "#660762", "#7D1D6B", "#871D62","#952664", "#AE2A5E", "#CA3F56", "#DB5854", "#DE5435", "#E45B2F", "#E86328", "#ED6B21", "#ED8861", "#F4A472", "#F4C372", "#F4D17B", "#ECC763", "#E8B94B", "#D4A432"))
...
circos.clear()
cairo_pdf(paste0(here::here(), "/Graficos/Estado/03_Movilidad laboral/2020/ChordDiagram de MTrab a nivel estatal.pdf"),
          width = 7, height = 7, 
...
cairo_pdf(paste0(here::here(),"/Graficos/Estado/03_Movilidad laboral/2020/ChordDiagram de MTrab para cada estado.pdf"),
          width = 8,
          height = 8, 
          family = "Montserrat Medium",
          fallback_resolution = 400,
...
cairo_pdf(paste0(here::here(), "/Graficos/Estado/03_Movilidad laboral/2020/Etiquetas a nivel estatal.pdf"), 
          width = 7, height = 8, 
          fallback_resolution = 400,
...
```{r, echo = FALSE}
load(file = paste0(here::here(), "/Output/Estado/03_Movilidad laboral/2020/Indicadores de MTrab a nivel estatal 2020.RData"))
...
cairo_pdf(paste0(here::here(), "/Graficos/Municipio/03_Movilidad laboral/2020/ChordDiagram de MTrab desagregado por municipio.pdf"),
          width = 15,
          height = 10, 
          family = "Montserrat Medium",
          fallback_resolution = 400,
...
cairo_pdf(paste0(here::here(), "/Graficos/Municipio/03_Movilidad laboral/2020/Etiquetas a nivel municipal.pdf"), 
          width = 7, height = 10, 
          fallback_resolution = 400,
          family = "montserrat", onefile = TRUE)
for(i in 1:length(estados)){
...
```{r, echo = FALSE}
load(file = paste0(here::here(), "/Output/Municipio/03_Movilidad laboral/2020/Indicadores de movilidad laboral a nivel municipal 2020.RData"))
...
**Matriz de migración laboral hace 5 años a nivel municipal, 2020**  
...
```{r, eval = FALSE}
pdf(paste0(here::here(), "/Graficos/Municipio/03_Movilidad laboral/2020/ChordDiagram de Mtrab desagregado a nivel intramunicipal.pdf"), width = 20, height = 20)
for(i in 1:32){
# Paleta de colores
...
path = paste0(here::here(), "/Graficos/Municipio/03_Movilidad laboral/2020/Etiquetas a nivel intramunicipal.pdf")
...
```{r, echo = FALSE}
load(file = paste0(here::here(), "/Output/Municipio/03_Movilidad laboral/2020/Indicadores de MTrab a nivel intramunicipal 2020.RData"))
...
```{r, eval = FALSE}
pdf(paste0(here::here(), "/Graficos/Municipio/03_Movilidad laboral/2020/ChordDiagram de Mtrab desagregado a nivel intermunicipal.pdf"), width = 20, height = 20)
for(i in 1:32){
# Paleta de colores
groupColors <- setNames(viridis::viridis(length(unique(c(colnames(tabla1[[i]]), rownames(tabla1[[i]])))), option = "A"), 
                        nm = unique(c(colnames(tabla1[[i]]), rownames(tabla1[[i]]))))
...
path = paste0(here::here(), "/Graficos/Municipio/03_Movilidad laboral/2020/Etiquetas a nivel intermunicipal.pdf")
...
```{r, echo = FALSE}
load(file = paste0(here::here(), "/Output/Municipio/03_Movilidad laboral/2020/Indicadores de MTrab a nivel intermunicipal 2020.RData"))
