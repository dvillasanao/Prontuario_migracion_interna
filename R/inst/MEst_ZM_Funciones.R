...
totales <- function(tabla1, ZM) {
              lapply(1:length(ZM), function(x){
                ...
                full_join(
                  ...
                  rename("CVE_MUN" = "name",
                         "Entran por estudio" = "Total_Filas", # Población que entra a la entidad para estudiar
                         "Salen por estudio" = "Total") %>% # Población que sale de su entidad de residencia y entra a otra demarcación por motivos de estudio
                  select(`Entran por estudio`, CVE_MUN, `Salen por estudio`)
  })
}
